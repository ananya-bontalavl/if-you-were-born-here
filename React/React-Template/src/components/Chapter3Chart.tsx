...
import { GNI_DATA } from "../data/gni_processed"; // Adjust path as needed
...
export default function Chapter3Chart({ countryData }: Props) {
  ...
  const [clicks, setClicks] = useState(0); // Track 0, 1, 2, or 3 clicks
  const [isWorking, setIsWorking] = useState(false);

  // 1. Find the data for the specific country
  ...
  // 2. Determine current GNI based on clicks
  ...
  useEffect(() => {
    ...
    segments.forEach((seg, i) => {
      g.append("rect")
        ...
        .attr("fill", seg.color)
        .attr("opacity", 0.9)
        .attr("rx", i === 0 ? 4 : i === segments.length - 1 ? 4 : 0);
    ...
    g.append("line")
      .attr("x1", gniX).attr("x2", gniX)
      .attr("y1", -8).attr("y2", barHeight + 8)
      .attr("stroke", "#fff")
      .attr("stroke-width", 3)
    ...
    g.append("text")
      ...
      .text(clicks === 0 ? "$0" : `$${Math.round(currentGni).toLocaleString()}`);
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '15px' }}>
      ...
      <div style={{ 
        padding: '24px', borderRadius: '24px', backgroundColor: 'rgba(255,255,255,0.02)', 
        ...
        <button 
          onClick={handleWork}
          disabled={clicks >= 3 || isWorking}
          style={{
            padding: '14px 40px', borderRadius: '100px',
            background: clicks >= 3 ? '#111' : (isWorking ? '#333' : '#fff'),
