...
export default function Chapter3Chart2({ selectedCountry }: Props) {
  ...
  useEffect(() => {
    ...
    categories.forEach((cat, i) => {
      ...
      g.append("circle").attr("r", 5).attr("fill", color(cat));
      g.append("text").attr("x", 10).attr("y", 5).attr("fill", "#888").style("font-size", "15px").style("font-weight", "600").text(cat);
    });

    svg.append("g").attr("transform", `translate(0,${height})`).call(d3.axisBottom(x)).attr("color", "#666") .style("font-size", "20px");
    svg.append("g").call(d3.axisLeft(y).ticks(5)).attr("color", "#666").style("font-size", "20px");;
  }, [selectedCountry]);

  return (
    <div style={{ position: "relative" }}>
      <div ref={ref}></div>
      <div ref={tooltipRef} style={{ position: "fixed", background: "white", color: "black", padding: "8px", borderRadius: "4px", pointerEvents: "none", opacity: 0, zIndex: 1000, fontSize: "12px", boxShadow: "0 4px 10px rgba(0,0,0,0.3)" }} />
    </div>
  );
}